...
export const createGame = async () => {
  ...
  await kv.set(["game", gameId], {
    players: [gameMaster.id],
    distribution: { werwolf: 0, dorfbewohner: 0, hexe: 0, seherin: 0 },
  });
...
export const changeRoleDistribution = async (
  gameId: string,
  role: keyof Distribution,
) => {
  const game = await kv.get(["game", gameId]);
  switch (role) {
    case "werwolf":
...
function getShuffledRoleArrayFromDistribution(distribution: Distribution) {
  ...
  for (const key in distribution) {
    console.log(
      key,
      distribution[key as keyof Distribution],
      "getShuffledRoleArrayFromDistribution 1",
    );
